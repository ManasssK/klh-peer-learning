...
function detectIntent(q) {
  ...
  const casual = /^(hey|hi|hello|yo|sup|what'?s up|how('?s)? it going)/.test(s) || s.length <= 20;
  const platform = /(video|playlist|find|search|upload|profile|delete|comment|dashboard|browse|watch|create|navigate|how to|where|path|steps|link)/.test(s);
  const teach = /(explain|what is|how does|why|derive|prove|algorithm|complexity|example|code|formula|theorem)/.test(s);
  const reasoning = /(why|compare|analyze|evaluate|recommend|best way|should i|strategy|plan|roadmap|steps to|difference between)/.test(s);
...
function buildUserPrompt(question, style, platformCtx) {
  if (style === 'Navigate') {
    ...
    const featured = (platformCtx?.featured || [])
      .map((p, i) => `${i + 1}. "${p.title}" (${p.n} videos) -> /playlists/${p.id}`)
...
// --- Output normalizer: remove markdown artifacts (**, *, ```
...
export async function askAI(question, platformCtx) {
  ...
  const { text, model } = await callPerplexity({ prompt, system, reasoning: style !== 'Casual' && reasoning });
